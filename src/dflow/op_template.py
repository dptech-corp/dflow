...
class ScriptOPTemplate(OPTemplate):
    ...
    def get_memoize(self):
        if self.memoize_key is not None:
            ...
            return V1alpha1Memoize(key=self.memoize_key, local_vars_configuration=config, cache=V1alpha1Cache(config_map=V1ConfigMapKeySelector(name="dflow-config", local_vars_configuration=config)))
        return None

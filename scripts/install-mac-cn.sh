...
if [[ -n "$minikube_path" ]]; then
    ...
    machine_type=$(uname -m)
    if [[ "$machine_type" == "amd64" || "$machine_type" == "x86_64" ]]; then
        curl -o minikube -L "https://registry.npmmirror.com/-/binary/minikube/v1.25.2/minikube-darwin-amd64"
    elif [[ "$machine_type" == "arm64" ]]; then
        curl -o minikube -L "https://registry.npmmirror.com/-/binary/minikube/v1.25.2/minikube-darwin-arm64"
    else
        ERROR "Unrecognized machine architecture"
    fi
    if [[ $? != 0 ]]; then
...
wget https://gitee.com/deepmodeling/dflow/raw/master/manifests/quick-start-postgres-stable-cn.yaml
